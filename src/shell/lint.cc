...
namespace {
	...
	void check_name(
			const std::string& name, size_t line, const vocabulary& known, const names& bound, finding& out) {
	...
	void walk(const Qd::IAstNode* node, const vocabulary& known, names& bound, finding& out) {
		...
		switch (node->type()) {
			...
			case Type::IDENTIFIER:
				check_name(
						static_cast<const Qd::AstNodeIdentifier*>(node)->name(), node->line(), known, bound, out);
				return;
